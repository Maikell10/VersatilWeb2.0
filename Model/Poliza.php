...
class Poliza extends Conection
{
    ...
    {
        $sql = "SELECT idnom AS nombre, cod, act FROM ena WHERE cod = '$cod_vend'
                UNION
                SELECT nombre, cod, act FROM enp WHERE cod = '$cod_vend'
                UNION
                SELECT nombre, cod, act FROM enr WHERE cod = '$cod_vend'";
        $query = mysqli_query($this->con, $sql);
    ...
    {
        $sql = "SELECT SUM(comision.prima_com) AS prima_com, SUM(comision.comision) AS comision 
                FROM comision 
                INNER JOIN rep_com, poliza
                WHERE 
                comision.id_rep_com = rep_com.id_rep_com AND
    ...
    public function get_comision_asesor_by_id($id_poliza, $cod_vend)
    {
        $sql = "SELECT idnom AS nombre, gc_h_pago.montop, ftransf, ref, poliza.id_poliza
                ...
                gc_h_pago.id_gc_h = gc_h.id_gc_h AND
                gc_h.id_gc_h = gc_h_comision.id_gc_h AND
                comision.id_comision = gc_h_comision.id_comision AND
                comision.id_poliza = poliza.id_poliza AND
                gc_h_pago.cod_vend = ena.cod AND
                poliza.id_poliza = '$id_poliza' AND
                gc_h_pago.cod_vend = '$cod_vend'  
                ORDER BY `gc_h_pago`.`ftransf`  DESC ";
    ...
    {
        $sql = "SELECT nombre, enp.currency, enp.monto, gc_h_p.monto_p, f_pago_gc_r, gc_h_p.id_poliza, n_transf
                ...
                SELECT nombre, enr.currency, enr.monto, gc_h_r.monto_p, f_pago_gc_r, gc_h_r.id_poliza, n_transf
    ...
    public function get_distinct_fgc_exist_by_r($f_desde, $f_hasta, $asesor)
    {
    ...
    public function get_distinct_fgc_exist_by_p($f_desde, $f_hasta, $asesor)
    {
        $sql = "SELECT DISTINCT(MONTH(f_desdepoliza)) AS mes, YEAR(f_desdepoliza) AS anio FROM 
                poliza
                INNER JOIN dcia, enp
    ...
    public function get_no_birthdays_filter($asesor, $cia, $ramo, $t_poliza)
    {
        $f_hoy = date('Y-m-d');
        $asesorIn = '';
        if ($asesor != '') {
    ...
    public function editarUsuario($id_usuario, $nombre, $apellido, $ci, $zprod, $email, $seudonimo, $clave, $id_permiso, $asesor, $activo, $carga, $renov, $ccl, $cb, $cr, $cc, $cp)
    {
